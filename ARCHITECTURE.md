...
[proto](https://github.com/istio/operator/blob/release-1.3/pkg/apis/istio/v1alpha2/istiocontrolplane_types.proto) and
compiled to [Go
structs](https://github.com/istio/operator/blob/release-1.3/pkg/apis/istio/v1alpha2/istiocontrolplane_types.pb.go).
`IstioControlPlaneSpec` has pass-through fields to the Helm values.yaml API, but these are additionally validated through
a [schema](https://github.com/istio/operator/blob/release-1.3/pkg/apis/istio/v1alpha2/values/values_types.proto). 
...
compatible with the older charts in [istio/istio](https://github.com/istio/istio/tree/release-1.3/install/kubernetes/helm).
...
[IstioControlPlaneSpec proto](https://github.com/istio/operator/blob/release-1.3/pkg/apis/istio/v1alpha2/istiocontrolplane_types.proto),
...
[values_types.proto](https://github.com/istio/operator/blob/release-1.3/pkg/apis/istio/v1alpha2/values/values_types.proto).
...
indexed by minor [version](https://github.com/istio/operator/blob/release-1.3/pkg/version/version.go). This is because
...
[pkg/validate/validate.go](https://github.com/istio/operator/blob/release-1.3/pkg/validate/validate.go). These rules
...
[validate_values.go](https://github.com/istio/operator/blob/release-1.3/pkg/validate/validate_values.go)
...
[default profile](https://github.com/istio/operator/blob/release-1.3/data/profiles/default.yaml) is used. Each profile is defined as a
...
The CLI `mesh` command is implemented in the [cmd/mesh](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/)
subdirectory as a Cobra command with the following subcommands:

- [manifest](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/manifest.go): the manifest subcommand is used to generate, apply, diff or migrate Istio manifests, it has the following subcommands:
    - [apply](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/manifest-apply.go): the apply subcommand is used to generate an Istio install manifest and apply it to a cluster.
    - [diff](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/manifest-diff.go): the diff subcommand is used to compare manifest from two files or directories.
    - [generate](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/manifest-generate.go): the generate subcommand is used to generate an Istio install manifest.
    - [migrate](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/manifest-migrate.go): the migrate subcommand is used to migrate a configuration in Helm values format to IstioControlPlane format.
    - [versions](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/manifest-versions.go): the versions subcommand is used to list the version of Istio recommended for and supported by this version of the operator binary.
- [profile](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/profile.go): dumps the default values for a selected profile, it has the following subcommands:
    - [diff](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/profile-diff.go): the diff subcommand is used to display the difference between two Istio configuration profiles.
    - [dump](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/profile-dump.go): the dump subcommand is used to dump the values in an Istio configuration profile.
    - [list](https://github.com/istio/operator/blob/release-1.3/cmd/mesh/profile-list.go): the list subcommand is used to list available Istio configuration profiles.
